...
void griffin_apply_non_linear_layer(blst_fr *ctxt) {
  blst_fr tmp;
  blst_fr tmp_sqr;
  // y_0 = x_0^(1/d)
  griffin_addchain_alpha_inv(ctxt, ctxt);
  // y_1 = x_1^d
  memcpy(&tmp, ctxt + 1, sizeof(blst_fr));
  blst_fr_sqr(&tmp, &tmp);
  blst_fr_sqr(&tmp, &tmp);
  blst_fr_mul(&tmp, ctxt + 1, &tmp);
  memcpy(ctxt + 1, &tmp, sizeof(blst_fr));
  // y_2 = x_2 * [y_0 + y_1] + \
  //       alpha_2 * [y_0 + y_1] + \
  //       beta_2
  //   -> y_0 + y_1
  ...
  //   -> tmp^2 = (y_0 + y_1)^2
  blst_fr_sqr(&tmp_sqr, &tmp);
  //   -> tmp = alpha_2 * tmp
  //          = alpha_2 * (y_0 + y_1)
  blst_fr_mul(&tmp, &tmp, &ALPHA_2);

  //   -> tmp = tmp + BETA_2
  //          = alpha_2 * (y_0 + y_1) + BETA_2
  blst_fr_add(&tmp, &tmp, &BETA_2);
  //   -> tmp = tmp + (y_0 + y_1)^2
  //          = (y_0 + y_1)^2 + \
  //            alpha_2 * (y_0 + y_1) + \
  //            BETA_2
  blst_fr_add(&tmp, &tmp, &tmp_sqr);
  //   -> tmp = tmp * x_2
  //          = x_2 * [
  //              (y_0 + y_1)^2 + \
  //              alpha_2 * (y_0 + y_1) + \
  //              BETA_2
  //            ]
  blst_fr_mul(ctxt + 2, ctxt + 2, &tmp);
}

void griffin_apply_linear_layer(blst_fr *ctxt) {
  blst_fr tmp;

  // We apply the circular matrix Circ(2, 1, 1)
  // -> require 5 additions
  // Compute sum(state)
  blst_fr_add(&tmp, ctxt, ctxt + 1);
  blst_fr_add(&tmp, &tmp, ctxt + 2);
...
int griffin_add_constant(blst_fr *ctxt, int i_round_key) {
  for (int i = 0; i < GRIFFIN_STATE_SIZE; i++) {
    blst_fr_add(ctxt + i, ctxt + i, GRIFFIN_ROUND_CONSTANTS + i_round_key++);
  }
  return (i_round_key);
}
...
void griffin_apply_permutation(blst_fr *ctxt) {
  ...
  for (int i = 0; i < GRIFFIN_NB_ROUNDS; i++) {
